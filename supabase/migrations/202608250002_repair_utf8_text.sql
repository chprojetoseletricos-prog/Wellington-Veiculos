create or replace function private.repair_utf8_mojibake(input text)
returns text
language plpgsql
immutable
strict
...
begin
  ...
  loop
    execute format(
      'update %I.%I set %I = private.repair_utf8_mojibake(%I) where strpos(%I, chr(195)) > 0 or strpos(%I, chr(194)) > 0 or strpos(%I, chr(226)) > 0',
...
set value = to_jsonb(private.repair_utf8_mojibake(value #>> '{}'))
...
drop function private.repair_utf8_mojibake(text);
